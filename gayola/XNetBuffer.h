...
//#define IS_DIGIT(c) ((c) >= '0' && (c) <= '9')
	...
	static char x_str_255f(std::string s)
	{
		...
		int _posL = (int)s.find("&#");
		int _posR = (int)s.find(";", _posL);
		if (_posL != -1 && _posR != -1)
		{
...
public:
	...
	CxDChunk(const void* _dat, uint _size)
	{
		dat = NULL;
		len = 0;
		SetData(_dat,_size);
	...
	void SetData(const void* _dat,uint _size)
	{
		assert(_size > 0 && _dat);
		//dat.resize(_size);
		resize(_size);
...
public:
	...
	void WritePackage(const void* _dat, unsigned int _size,const char* _emark,int _emark_len)
	{
		...
		resize((uint)des.size() + _emark_len);
					   ...
					   const char* _emark,unsigned int _emark_len)
	{
		...
		for(int i=0;i<l;i++)
		{
			if(CxNetBufferHelper::x_str_first(_dat+i,_emark,_emark_len)){
				CxNetBufferHelper::xarray_uint8_append(des,X_CLRF2_C,(uint)strlen(X_CLRF2_C));
				i+=(_emark_len-1);
...
//		des.swap(dat);
	...
	void ReadPackageFix2(const char* _emark, int _emark_len)
	{
		...
		for(int i=0;i<l;i++)
		{
			...
			{
				CxNetBufferHelper::xarray_uint8_append(des,X_CLRF2,(uint)strlen(X_CLRF2));
				i+=3;
...
//		_storage.resize(65535);
	...
	uint getWriteSpace(unsigned int _asize=1) {
		uint  l = _total - _wpos;
		if (l < _asize) {
			l = _asize-l;
...
//		_storage.resize(_Size);
	...
	*/
	...
	返回 除尾标外的数据
	*/
	CxDChunk*  getChunkEMark2(const char* _endmark,uint _size)
	{
		std::lock_guard<std::mutex> lck(mtx_class);
		...
		if (_endmark == NULL) _endmark = "\r\n\r\n";
		if (_size < 1) _size = (uint)strlen(_endmark);
	...
	*/
	void WritefixEMark2(const char* _dat, unsigned int _size, const char* _emark, int _emark_len)
	{
		...
		if(_emark==NULL) _emark="\r\n\r\n";
		if (_emark_len < 1) _emark_len = (uint)strlen(_emark);
		...
		if (l > 0)
		{
			uint mss = (uint)strlen(X_CLRF2_C);
	...
	void EncodeEMark(const char* buf, uint size, const char* prefix, uint psize, const char* _emark, uint _emark_size)
	{
		xarray_uint8 _data;
		CxNetBufferHelper::xarray_uint8_append(_data, prefix, psize);
		CxNetBufferHelper::xarray_uint8_append(_data, buf, size);
		WritefixEMark2((const char*)&_data[0], (int)_data.size(),_emark,_emark_size);
	}
