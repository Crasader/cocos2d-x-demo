...
int xnet_send(xnet_sock s, const char * lpBuf, int size)
{
	...
	r= send(s, lpBuf, size, 0);
	if (r == SOCKET_ERROR || r==size) return r;
	while (r < size)
	{
//		std::this_thread::sleep_for(std::chrono::milliseconds(100));
		int _r= send(s, lpBuf, size, 0);
		if (_r == SOCKET_ERROR) return _r;
		r += _r;
	}
	return r;
}
...
struct hostent * xnet_gethostbyname(const char* host_name)
{
	DWORD dwError;
...
struct hostent * xnet_gethostbyname(const char* host_name)
{
	return gethostbyname2(host_name, 0);
}
